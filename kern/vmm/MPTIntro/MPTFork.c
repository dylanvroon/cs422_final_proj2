...
void setup_child_pdir(unsigned int parent, unsigned int child) 
{
    ...
    //NOTE: Fixed this ^
    unsigned int pde_index, pte_index, ptbl_entry, chi_page_index, page_index, perm, vaddr;
        ...
        if (get_pdir_entry(parent,pde_index) != 0) {
            ...
            for (pte_index = 0; pte_index < 1024; pte_index++) {
                if (get_ptbl_entry(parent, pde_index, pte_index) != 0) {
                    ...
                    set_ptbl_entry(parent, pde_index, pte_index, page_index, perm);
                    set_ptbl_entry(child, pde_index, pte_index, page_index, perm);
                } else {
